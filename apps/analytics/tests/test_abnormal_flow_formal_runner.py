...
def test_portfolio_engine_v2_integration(run_env: dict[str, Path]) -> None:
    from unittest.mock import patch

    import duckdb
    import schurfer_analytics.abnormal_flow_formal_runner as runner_module
    from schurfer_analytics.abnormal_flow_snapshots import _row_to_decision, _row_to_outcome
    from schurfer_analytics.portfolio_engine_v2 import simulate_portfolio_v2

    fire1 = _decision("FIRE1", oi_growth=20.0)
    fire2 = _decision("FIRE2", oi_growth=20.0)

    outcomes = {
        fire1.route_key(): Outcome(
            exchange="bybit",
            market_type="linear",
            native_market_id="FIRE1",
            capture_version="v1",
            symbol="FIRE1",
            decision_at=fire1.decision_at,
            entry_price=100.0,
            exit_price=110.0,
        ),
        fire2.route_key(): Outcome(
            exchange="bybit",
            market_type="linear",
            native_market_id="FIRE2",
            capture_version="v1",
            symbol="FIRE2",
            decision_at=fire2.decision_at,
            entry_price=100.0,
            exit_price=120.0,
        ),
    }
    ...
    def chunks(*args: Any, **kwargs: Any) -> Any:
        return iter([[_bar("FIRE1")], [_bar("FIRE2")]])

    def decisions(bars: list[MinuteBar], **kwargs: Any) -> list[DecisionFeatures]:
        if bars[0].symbol == "FIRE1":
            return [fire1]
        return [fire2]
    ...
    # Read episodes
    ep_rows = (
        db.read_parquet(str(run_dir / "episodes_snapshot.parquet")).order("decision_at").fetchall()
    )
    snapshot_episodes = [_row_to_decision(r) for r in ep_rows]

    # Read outcomes
    out_rows = (
        db.read_parquet(str(run_dir / "outcomes_snapshot.parquet")).order("decision_at").fetchall()
    )
    snapshot_outcomes = {_row_to_outcome(r).route_key(): _row_to_outcome(r) for r in out_rows}
