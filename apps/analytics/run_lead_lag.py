...
async def run():
    ...
    try:
        async with engine.connect() as conn:
            ...
            print(f"Fetched {len(df)} aggregated rows (seconds with liquidations).")
            df_pivot = df.pivot(index="sec", columns="exchange", values="vol_usd").fillna(0)

            if "binance" in df_pivot.columns and "bybit" in df_pivot.columns:
                print("Calculating Time-Lagged Cross-Correlation (TLCC)...")
                lags = range(-10, 11)
                corrs = []
                for lag in lags:
                    corr = df_pivot["binance"].corr(df_pivot["bybit"].shift(lag))
                ...
                print("Missing one of the exchanges for correlation.")
    finally:
