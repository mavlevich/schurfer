...
@dataclass(frozen=True)
class DerivativesRegimeFeasibilityReport:
...
async def build_feasibility_report(
    ...
    *,
    ...
    code_revision: str,
    working_tree_dirty: bool,
) -> DerivativesRegimeFeasibilityReport:
    ...
    return DerivativesRegimeFeasibilityReport(
        manifest=DerivativesFeasibilityManifest(
...
def render_json(report: DerivativesRegimeFeasibilityReport) -> str:
    return json.dumps(json_ready(asdict(report)), indent=2, sort_keys=True, allow_nan=False)


def render_markdown(report: DerivativesRegimeFeasibilityReport) -> str:
    ...
    lines = [
        ...
        "",
        (
            "> Feasibility only. No outcome, return, or PnL association is computed "
...
async def _run(args: argparse.Namespace) -> str:
    ...
    if not db_url:
        raise ValueError("DATABASE_URL is required for derivatives-regime-feasibility-report")
