...
- **`shared_identity_key_across_bases`**: the same canonical instrument
  resolves under two different `base` strings (this is the direct integrity
  check for the exact "牛来 vs NIULAI" ambiguity the discussion that
  motivated this report ran into -- they are tracked as two separate `base`
  rows today, and this report either confirms they share an identity_key
  under the hood, in which case they should be treated as one, or confirms
  they resolve to genuinely different instruments, in which case the
  separate tracking is correct).
- **`base_maps_to_multiple_instruments`**: the same `base` string resolves
  to more than one distinct instrument across its own episodes (spot vs
  perp, a relist, a ticker collision) -- the scanner's per-base aggregation
  silently mixes these into one row today.

Identity classification reuses `source_lead.py`'s `_identity_reason`
fail-closed discipline, generalized: pump events are not limited to USDT
swaps the way source-lead routes are, so the swap/USDT-only checks are
dropped and only the universal checks are kept (conflict flag, key/symbol
presence, base match). An observation this report cannot resolve
(`missing_identity`, `identity_conflict`, `base_mismatch`) is excluded from
collision detection rather than guessed either way -- fail-closed, same as
every other identity check in this codebase.
...
PUMP_RECURRENCE_INTEGRITY_REPORT_VERSION = "pump_recurrence_integrity_report_v1"
...
# both included deliberately: the report's own collision detector is what
# decides whether they are the same instrument, not this list.
...
class IdentityCollision:
    kind: str  # "shared_identity_key_across_bases" | "base_maps_to_multiple_instruments"
    bases: tuple[str, ...]
...
def interval_histogram(episodes: tuple[Episode, ...]) -> dict[str, int]:
    ...
    sorted, single base). This is descriptive evidence for *why* merging is
    needed -- it is not itself the merge. `overlapping` (a negative gap) is
    its own bucket rather than folded into `under_5m`, since a negative gap
    signals a data-ordering anomaly worth surfacing on its own, not ordinary
    rapid recurrence.
    """
...
def merge_episodes_into_regimes(
    ...
    when the gap between one episode's `last_seen_at` and the next episode's
    `first_seen_at` is shorter than `cooldown`.
    ...
    for prev, curr in pairwise(episodes):
        if curr.first_seen_at - prev.last_seen_at < cooldown:
...
def detect_identity_collisions(
    ...
    """Two independent collision checks, deliberately NOT the same grouping:
    ...
    - `shared_identity_key_across_bases`: the exact same `identity_key`
      ...
      means one instrument is tracked under two ticker labels -- the 牛来
      vs NIULAI scenario this report exists to catch.
    ...
    Grouping by base alone, across every exchange, is deliberately NOT done
    here: `identity_key`'s venue prefix means a single, correctly cross-
    listed instrument (e.g. ADA on binance/bybit/gate/...) always produces a
    distinct key per exchange by construction. That is normal multi-venue
    listing, not a collision -- comparing across exchanges this way was
    tried and produces hundreds of false positives on the real population
    (confirmed against production data before this report shipped).
    """
    resolved = [observation for observation in observations if identity_reason(observation) is None]
    ...
    for observation in resolved:
        ...
        if key is None:  # pragma: no cover - identity_reason already excludes this
            continue
