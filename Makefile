...
# Reads the window HYP-022 declared held out, exactly once. The window is a
# constant in the module, not a flag, so this target cannot be pointed
# elsewhere by accident or otherwise.
prod-scaled-exit-replication:
	@test -f .env.prod || (echo "ERROR: .env.prod not found. Copy .env.prod.example and fill in." && exit 1)
	@$(_PROD) run --rm --no-deps --entrypoint scaled-exit-replication analytics \
		--code-revision="$$(git rev-parse HEAD)" \
		$$(test -z "$$(git status --porcelain)" \
			&& printf '%s' '--no-working-tree-dirty' \
			|| printf '%s' '--working-tree-dirty') $(ARGS)
