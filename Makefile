...
# HYP-027. The contract path is required for the same reason as HYP-023's: the
# window, the horizon, the group cutoffs and the floors all come from it. Pass
# MANIFEST to freeze the sample, which is what makes "one read" checkable rather
# than a sentence the report prints about itself.
prod-pump-age-resolution-study:
	@test -f .env.prod || (echo "ERROR: .env.prod not found. Copy .env.prod.example and fill in." && exit 1)
	@test -n "$(CONTRACT)" || (echo "ERROR: set CONTRACT=docs/research/contracts/<file>.json" && exit 1)
	...
	@$(_PROD) run --rm --no-deps -v $(PWD)/$(CONTRACT):/contract.json \
		...
		$(if $(MANIFEST),--sample-manifest /samples/$(MANIFEST),) \
		--code-revision="$$(git rev-parse HEAD)" \
		$$(test -z "$$(git status --porcelain)" \
			&& printf '%s' '--no-working-tree-dirty' \
			|| printf '%s' '--working-tree-dirty') $(ARGS)
